...
Check All ABR Services
    ...
    Run Keyword and Continue On Failure    Check ABR Baseline Verison    ${ABR-IP}    ${username}    ${password}
    Run Keyword and Continue On Failure    Fetch Xepa IP and Check reachability for XEPA IP    ${ABR-IP}    ${username}    ${password}
    Run Keyword and Continue On Failure    Count of ABR Services Running    ${ABR-IP}    ${username}    ${password}
    Run Keyword and Continue On Failure    Check vrf default route    ${ABR-IP}    ${username}    ${password}
    Run Keyword and Continue On Failure    Check CIBR's Connectivity    ${ABR-IP}    ${username}    ${password}
    Run Keyword and Continue On Failure    Check BGP connectivity with neighbours    ${ABR-IP}    ${username}    ${password}
    Run Keyword and Continue On Failure    Check Undlay-vrf statistics    ${ABR-IP}    ${username}    ${password}
...
Fetch Xepa IP and Check reachability for XEPA IP
    [Arguments]    ${ABR-IP}    ${username}    ${password}
    ${Xepa_Command}=    Execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ABR-IP} 'cat /etc/hosts'
    log    ${Xepa_Command}
    ${grep_xepa_IP}=   Execute command    echo "${Xepa_Command}" | grep xepa.control | cut -f 1 -d" "
    log    ${grep_xepa_IP}
...
Check ABR Baseline Verison
   [Arguments]    ${ABR-IP}    ${username}    ${password}
   ${Baseline_Command}=    Execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ABR-IP} 'cat /var/lib/portables/abr/.vars | egrep "BASELINE|VERSION"'
...
Count of ABR Services Running
   [Arguments]    ${ABR-IP}     ${username}    ${password}
   ${ABR_Services}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ABR-IP} 'systemctl list-units '\\'abr-*\\'
   log    ${ABR_Services}
   ${running_services}=    execute command    echo "${ABR_Services}" | grep running | wc -l
   log    ${running_services}
   ${exited_service}=    execute command    echo "${ABR_Services}" | grep exited | wc -l
   log    ${exited_service}
...
Check vrf default route
   [Arguments]    ${ABR-IP}    ${username}    ${password}
   ${vrf_Services}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ABR-IP} ip route show vrf a2b2a | grep ^default
   log    ${vrf_Services}
   should contain    ${vrf_Services}    ${default}    msg=Default route is not found for brkout interface, please check vrf a2b2a

Check CIBR's Connectivity
   [Arguments]    ${ABR-IP}    ${username}    ${password}
   ${CIBR_Connect}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ABR-IP} 'ss | grep control | grep ESTAB'
   log    ${CIBR_Connect}
   ...
   ELSE IF    ${cibr_count} < 2
               Fail    log    TCP:CIBR1 and CIBR2 connections are down
   ELSE
...
Check BGP Stats
    [Arguments]    ${ABR-IP}    ${username}    ${password}
    ${BGP_stats}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ABR-IP} 'vtysh -c "sh ip bgp" | grep Displayed'
    log    ${BGP_stats}
