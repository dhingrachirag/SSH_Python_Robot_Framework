...
Check Running Services
    ...
    ...    Log Error    Please check ABR services as less running services are running
    ...
    ...    Log Error    Something wrong with ABR Services, Please check
...
Check CBR Connection
    ...
    log    ${BGP_stats}
    ${cbrVTEPRoutePresent}=    execute command    echo "${BGP_stats}" | grep -A 1 -o -E "$(echo ${cbrvtep} | sed 's/,/|/g')" | wc -l
    log    ${cbrVTEPRoutePresent}
    ${Filtered_BGP}=    execute command    echo "${BGP_stats}" | grep -A 1 "${cbrvtep}/32"
    log    ${Filtered_BGP}
    ${CBR_Route1}=    execute command    echo "${Filtered_BGP}" | grep -E "169\.254\.201\.(33|101)" | wc -l
    ${CBR_Route2}=    execute command    echo "${Filtered_BGP}" | grep -E "169\.254\.201\.(34|101)" | wc -l
    ${CBR_Route1}=    Set Variable If    ${CBR_Route1} > 0    1    0
    ${CBR_Route2}=    Set Variable If    ${CBR_Route2} > 0    1    0
    ${CBR_Route1}=    Evaluate    int(${CBR_Route1})
    ${CBR_Route2}=    Evaluate    int(${CBR_Route2})
    ${Total_Next_Hops}=    Evaluate    ${CBR_Route1} + ${CBR_Route2}
    log    Next-hop 169.254.201.33 count: ${CBR_Route1}
    log    Next-hop 169.254.201.34 count: ${CBR_Route2}
    Run Keyword If    ${Total_Next_Hops} == 2
    ...    Log    CBR-BGP: CBR is connected on 2 routes

    Run Keyword If    ${Total_Next_Hops} == 0
    ...    Fail    CBR-BGP: CBR is completely disconnected

    Run Keyword If    ${Total_Next_Hops} == 1 and ${CBR_Route1} == 0
    ...    Fail    CBR-BGP: BR1 is disconnected

    Run Keyword If    ${Total_Next_Hops} == 1 and ${CBR_Route2} == 0
    ...    Fail    CBR-BGP: BR2 is disconnected

    Run Keyword If    ${Total_Next_Hops} != 2 and ${Total_Next_Hops} != 0 and ${Total_Next_Hops} != 1
    ...    Fail    CBR-BGP: Issue with CBR connectivity
...
Check which of the AAA is Master
    ...
    log    ${AAA_master}
    ${AAA_master1}=    Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} cat /usr/local/var/apex/jail/proc/viep
    log    ${AAA_master1}
# Replace apostrophe to prevent syntax error
    ${AAA_master1}=    Replace String    ${AAA_master1}    '    ""
# Check if the output is empty
    ${AAA_length}=    Get Length    ${AAA_master1}
    IF    ${AAA_length} == 0
         Fail    Server is not reachable
    ELSE
         Log    Server is running as ${AAA_master1}
    END
...
Check Nanames from XEPA and connectivity with each of the ABRs
   ...
   ${XEPA_keepalive}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "tac xepa.log | awk '/keepalive/ && /${XEPA_namesoutput}/{print \\\$3; exit}'"
   log    ${XEPA_keepalive}
   sleep    10s
   ${XEPA_keepalive1}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "tac xepa.log | awk '/keepalive/ && /${XEPA_namesoutput}/{print \\\$3; exit}'"
   log    ${XEPA_keepalive1}
...
Check Collector ID in Collector
    ...
    @{ALLOWED}=   create list    ATL01    DFW01    EWR01    FRA04    FRA08    IAD01    LAX01    LON01    MAD01    MEL01    ORD01    PAR01    QRO01    SYD01    TYO01    YMQ01    GRU01    DUB01    LON02    DEN01
...
Check Account Name in Collector
    ...
    log    ${colout1}
    @{USlist}    create list    ATL01    DFW01    EWR01   IAD01    LAX01    ORD01    QRO01    YMQ01    DEN01
...
Check All CUST-RSX Services
    [Arguments]    ${ip}    ${username}    ${password1}
    ${password}=    Get Decrypted Text    ${password1}
    log    ${password}
    Establish Connection and Log In to AMB LAX directly
    run keyword and continue on failure    Check CUST-RSX answerx Services    ${ip}    ${username}    ${password}
    run keyword and continue on failure    Find DNS IP to check Internet Connectivity CUST-RSX   ${ip}    ${username}    ${password}
    run keyword and continue on failure    Check TCP/BGP Statistics CUST-RSX   ${ip}    ${username}    ${password}
    run keyword and continue on failure    Check Public DNS for CUST-RSX   ${ip}    ${username}    ${password}
    run keyword and continue on failure    Check CustRSX Namespace status service check    ${ip}    ${username}    ${password}
    run keyword and continue on failure    Check Advertised anycast IPs towards ABR    ${ip}    ${username}    ${password}
    run keyword and continue on failure    Verify Default route check from BR 172.31.11.1 routes    ${ip}    ${username}    ${password}
    run keyword and continue on failure    Verify Default route check from BR 172.31.11.2 routes    ${ip}    ${username}    ${password}
    close all connections

Check CUST-RSX answerx Services
   [Arguments]    ${ip}    ${username}    ${password}
   ${RSX_IP}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} ip netns exec custrsx systemctl status answerx
   log    ${RSX_IP}
   should contain    ${RSX_IP}    ${running1}    msg=Something wrong please check the answerx service    values=False
#   should contain    ${RSX_IP}    ${success}   msg=Something wrong please check the answerx service   values=False

Find DNS IP to check Internet Connectivity CUST-RSX
   [Arguments]    ${ip}    ${username}    ${password}
   ${RSX_IP}=    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} ip -br a | grep outgoing | awk '{print $3}' | cut -d '/' -f1
   log    ${RSX_IP}
   ${resp}    execute command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "ping 1.1.1.1 -I ${RSX_IP} -c 2"
   should not contain    ${resp}    ${Destination_Host_Unreachable}    msg=Internet not working    values=False
   should not contain    ${resp}    ${Something_is_wrong}    msg=Internet not working    values=False
...
Check TCP/BGP Statistics CUST-RSX
    [Arguments]    ${ip}    ${username}    ${password}
    Check BGP Status Over SSH   ${ip}    ${username}    ${password}

Check BGP Status Over SSH
    [Arguments]    ${ip}    ${username}    ${password}
    FOR    ${vrf}    IN    @{VRFS}
        ${cmd_state}=        Set Variable    'vtysh -N ${VTYSH_NODE} -c "show bgp vrf ${vrf} summary" | grep -A2 State' | awk 'NR>1 {print $NF}'
        ${state_output}=     Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} ${cmd_state}
        ${summary_output}=   Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} 'vtysh -N ${VTYSH_NODE} -c "show bgp vrf ${vrf} summary"'

        ${is_down}=          Run Keyword And Return Status    Should Contain Any    ${state_output}    Idle    Active    Connect    Down
        ${has_neighbor}=     Run Keyword And Return Status    Should Contain    ${summary_output}    Neighbor

        IF    ${is_down}
            Log To Console    🔴 BGP is DOWN in VRF ${vrf}
        ELSE IF    ${has_neighbor}
            Log To Console    ✅ BGP is UP in VRF ${vrf}
        ELSE
            Log To Console    ⚠️  Unable to retrieve BGP status for VRF ${vrf} (may not exist or command error)
        END
    END

Check Network Namespace
    [Arguments]    ${ip}    ${username}    ${password}
    ${netns_output}=    Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "ip netns list | grep -w custrsx || echo 'not found'"
    Should Not Contain    ${netns_output}    not found    msg=Network namespace custrsx does not exist    values=False

Check Malware Domain
    [Arguments]    ${ip}    ${username}    ${password}
    ${status}=    Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "ip netns exec custrsx dig -b 127.0.0.100 +notcp +tries=1 +time=3 maldom.rc.akadnstest.com @127.0.0.99 | grep status | awk '{print \$6}'"
    Should Contain    ${status}    NOERROR    msg=There is an issue in malware domain    values=False
    Log To Console    ✅ malware domain working fine

Check SafeSearch Domain
    [Arguments]    ${ip}    ${username}    ${password}
    ${domain}=    Set Variable    youtube.com
    ${status}=    Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "ip netns exec custrsx dig -b 127.0.0.101 +notcp +tries=1 +time=3 youtube.com @127.0.0.99 | grep 'youtube.com' | grep -v ';' | grep 'youtube.com' | awk '{print \$5}' | grep ${domain}"
    Should Contain    ${status}    restrict.${domain}.    msg=There is an issue in safesearch    values=False
    Log To Console    ✅ safesearch domain working fine

Check RAW DNS
    [Arguments]    ${ip}    ${username}    ${password}
    ${status}=    Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} "ip netns exec custrsx dig -b 127.0.0.102 +notcp +tries=1 +time=3 youtube.com @127.0.0.99 | grep status | awk '{print \$6}'"
    Should Contain    ${status}    NOERROR,    msg=There is an issue in RAW DNS    values=False
    Log To Console    ✅ RAW domain working fine

Check Public DNS for CUST-RSX
    [Arguments]    ${ip}    ${username}    ${password}
    Run keyword and continue on failure    Check Network Namespace    ${ip}    ${username}    ${password}
    Run keyword and continue on failure    Check Malware Domain    ${ip}    ${username}    ${password}
    Run keyword and continue on failure    Check SafeSearch Domain    ${ip}    ${username}    ${password}
    Run keyword and continue on failure    Check RAW DNS    ${ip}    ${username}    ${password}

Check Advertised anycast IPs towards ABR
    [Arguments]    ${ip}    ${username}    ${password}
    Check advertized anycast IPs for all vrf    ${ip}    ${username}    ${password}

Check advertized anycast IPs for all vrf
    [Arguments]    ${ip}    ${username}    ${password}
    FOR    ${vrf}    IN    @{VRFS1}
#        ${failed_vrfs}=    Create List
        ${cmd_state}=        Set Variable    'vtysh -N ${VTYSH_NODE} -c "show ip bgp vrf ${vrf}" | grep -E "^\*> +194\.35\.39\.(112|113|115|116|118|119)/32" | wc -l'
        ${state_output}=     Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} ${cmd_state}
        ${anycast_count}=    Convert To Integer    ${state_output}
        Run Keyword And Continue On Failure    Run Keyword If    ${anycast_count} < 6     Fail    Anycast IP check failed for VRFs:${vrf}
        Log    Found ${anycast_count} advertised anycast IPs in VRF ${vrf}
#        Run Keyword If    ${failed_vrfs}    Fail    Anycast IP check failed for VRFs: @{failed_vrfs}
    END



Verify Default route check from BR 172.31.11.2 routes
    [Arguments]    ${ip}    ${username}    ${password}
    ${cmd_state}=        Set Variable    'vtysh -N ${VTYSH_NODE} -c "sh ip bgp neighbors 172.31.11.2 routes"'
    ${state_output}=     Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} ${cmd_state}

    ${default_route}=    Set Variable    ${EMPTY}

    @{lines}=    Split To Lines    ${state_output}
    FOR    ${line}    IN    @{lines}
        ${is_default_route}=    Run Keyword And Return Status    Should Match Regexp    ${line}    .*0\\.0\\.0\\.0/0.*
        ${valid_entry}=    Run Keyword And Return Status    Should Match Regexp    ${line}    ^\\*.*
        ${is_match}=    Evaluate    ${is_default_route} and ${valid_entry}
        Run Keyword If    ${is_match}    Set Test Variable    ${default_route}    ${line}
    END

    Should Not Be Empty    ${default_route}    Default route (0.0.0.0/0) is missing from BGP table
    Log    Default route found: ${default_route}

Verify Default route check from BR 172.31.11.1 routes
    [Arguments]    ${ip}    ${username}    ${password}
    ${cmd_state}=        Set Variable    'vtysh -N ${VTYSH_NODE} -c "sh ip bgp neighbors 172.31.11.1 routes"'
    ${state_output}=     Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} ${cmd_state}
...
Check CustRSX Namespace status service check
    [Arguments]    ${ip}    ${username}    ${password}
    ${status}=    Execute Command    ${ssh_command} ${password} ${ssh_o_command} ${username}@${ip} systemctl list-units --type=service | grep custrsx-*
    log    ${status}
    ${output}=    Convert To String    ${status}
    ${lines}=    Split To Lines    ${output}
    FOR    ${line}    IN    @{lines}
        Should Contain    ${line}    active    msg=Service is not active: ${line}
    END
    Log    All custrsx services are in active state

CUST-RSX Services Check
    [Arguments]    ${site}
    ${users_json_path}=    set variable    ${EXECDIR}/${Common_Path}/${site}
    ${the file as string}=    operatingsystem.get file    ${users_json_path}
    ${parsed}=    Evaluate  json.loads("""${the file as string}""")    json
    ${properties}=  Set Variable  ${parsed["DE Site"]["CUST-RSX"]}
    Log  ${properties}
    @{Failed_RSXs}=    Create List
    @{Passed_RSXs}=    Create List
    set test variable    ${check_status}    ${0}
    ...
    FOR    ${key}    IN    @{properties}
         ${sub dict}=    Get From Dictionary    ${properties}    ${key}
         Log  ${sub dict}
         ${status}=    run keyword and return status    Check All CUST-RSX Services    ${sub dict}[ip]   ${sub dict}[username]   ${sub dict}[password1]
         log    ${status}
         run keyword and continue on failure    Run keyword if    ${status} == ${False}    fail    CUST-RSX services report is not fine. Please Check 'Check All CUST-RSX Services' Keywords for Detailed Investigation
         ${passed}    set variable if    ${status} == ${True}    ${True}
         log    ${passed}
         ${failed}    set variable if    ${status} == ${False}   ${False}
         log    ${failed}
         ...
         END
    END
    Log Many    Passed CUST-RSXs: ${Passed_RSXs}   Passed CUST-RSXs Count: ${pass}
   IF      ${fail} == ${check_status}
            log    All CUST-RSX's are passing.There is no Failure
    ELSE
           run keyword and continue on failure    Fail    Failed CUST-RSX's    Log Many    Failed CUST-RSXs: ${Failed_RSXs}    Failed CUST-RSXs Count: ${fail}    Please Check CUST-RSX Services in logs for ${Failed_RSXs}
